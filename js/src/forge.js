/* WebBlocks_Utils/ChosenSelect WebBlocks */
...
function move(tagEvent, tag) {
 ...
 /* WebBlocks_Utils/InfiniteScroll WebBlock */
 function InfiniteListInit(buttonId, LoadMoreId, LoadingMoreId) {
    $(window).on('scroll.' + LoadMoreId, function() { /* Binding to scroll using namespace */
        if($(window).scrollTop() + $(window).height() > $(document).height() - ($(window).height() * 0.30)) {
            var button = $('#' + buttonId);
            var LoadMore = $('#' + LoadMoreId);
            var LoadingMore = $('#' + LoadingMoreId);

            if($(LoadMore).is(':visible')) { /* Only if the element it's visible on screen (multiple infinite scrolls in the screen, e.g., client tabs) */
                $(window).off('scroll.' + LoadMoreId); /* Unbind */
 ...
 /* WebBlocks_Utils/InfiniteScroll_Custom WebBlock */
 function OSInfiniteListInit(widgetId, LoadMoreId, LoadingMoreId) {
    $(window).scroll(function() {
           if($(window).scrollTop() + $(window).height() > $(document).height() - ($(window).height() * 0.30)) {
               ...
               $(LoadMore).toggle();
               $(LoadingMore).toggle();               
               OsNotifyWidget(widgetId, "");           
           }
    });
}
/***** forge module *****/
/* ForgeFlow/Component_Details */
var timerStart = Date.now();
/* ForgeFlow/Component_List */
...
/* ForgeFlow/ProjectUpload WebScreen */
// Prevent default drag to page behavior
...
function Loading() {
    clearInterval(pulse);
    $('.osicon-package').attr('pulse',1);
    pulse = setInterval( function() {
             var v = (parseFloat(parseFloat($('.osicon-package').attr('pulse')).toFixed(1)) - 0.1);
             $('.osicon-package').attr('pulse',v <= -1 ? 1: v);
             $('.osicon-package').css('opacity', Math.abs(v));
          }, 150);
}

function ValidateFileInput(ComponentFileUpload, url, GUID, continueId, ieId) {
    var ComponentType = $("#" + ComponentFileUpload).val();
    ComponentType = ComponentType.substr((ComponentType.length - 4),4).toLowerCase();

    if (! validateType(ComponentType)) {
        $(".ShowErrorMessage").click();
    } else {
        if ($.browser.msie  && parseInt($.browser.version, 10) <= 9) {
            ...
            fd.append('GUID', GUID);
            sendFileToServer(fd, url,continueId);
        }
    }
}
function IsValidFileDrop(Filename) {
    ComponentType = Filename.substr((Filename.length - 4),4).toLowerCase();
...
function sendFileToServer(formData, url, continueId) {
    ...
    processData: false,
        ...
        success: function(data){
            Loading();
            $('.UploadStatus').text("Processing upload...");
            $('#'+continueId).click();
        }
    }); 
}
...
function onDrop(event, uploadUrl, GUID, continueId) {
    event.preventDefault();
    $(".DropZone").removeClass("OnDrag");
    if(IsValidFileDrop(event.dataTransfer.files[0].name)) {
        var fd = new FormData();
        var files = event.dataTransfer.files
        fd.append('file', files[0]);
        fd.append('GUID', GUID);
        sendFileToServer(fd, uploadUrl, continueId);
    }
}
...
if ($.browser.msie  && parseInt($.browser.version, 10) <= 9) {
    $('.INVISIBLEIE9').css('visibility', "hidden");
    $('.HIDEIE9').hide();
}
...
var onDragEnter = function(event) {
    event.preventDefault();
    $(".DropZone").addClass("OnDrag"); 
}, 

onDragOver = function(event) {
    event.preventDefault(); 
    if(!$(".DropZone").hasClass("OnDrag"))
        $(".DropZone").addClass("OnDrag");
}, 
...
/* ForgeFlow/ProjectUploadDependencies WebScreen */
window.addEventListener("dragover",function(e){
  e = e || event;
  e.preventDefault();
},false);
...
/* ForgeFlow/Header_Component WebBlock */
osjs(document).ready(function() {
    osjs(".component-icon-big").children().css("max-width","");
});
/* WebBlocksUtilFlow/FAQ WebBlock */
    $(document).ready(function(){
        $('.wnEntry > a').click(function(event){
            event.preventDefault();
            var self = $(this);
            if (self.hasClass('open')) {
                self.removeClass('open');
                self.parent().find('.FAQdescription').slideUp();
            } else {
                self.addClass('open');
                self.parent().find('.FAQdescription').slideDown();
            }
        });
    });
/* WebBlocksUtilFlow/ProjectEditDetails_Block WebBlock */
// Prevent default drag to page behavior
window.addEventListener("dragover",function(e){
  e = e || event;
  e.preventDefault();
...
function onDrop(event, uploadUrl, ScreenshotGUID, continueId) {
    event.preventDefault();
    $(".DropZone").removeClass("OnDrag");
    if(IsValidFileDrop(event.dataTransfer.files[0].name)) {
        var fd = new FormData();
        var files = event.dataTransfer.files
        fd.append('file', files[0]);
        fd.append('GUID', ScreenshotGUID);
        sendFileToServer(fd, uploadUrl, continueId);
    }
}

function ValidateFileInput(ComponentFileUpload, url, ScreenshotGUID, continueId, ieId) {
    
    var ComponentType = $("#" + ComponentFileUpload).val();
    ComponentType = ComponentType.substr((ComponentType.length - 4),4).toLowerCase();

    if (! validateImageType(ComponentType)) {
        $(".ShowErrorMessage").click();
    } else {
        if ($.browser.msie  && parseInt($.browser.version, 10) <= 9) {
            $("#"+ieId).click();
            ...
            var fd = new FormData();
            var files = $('input[type=file].ScreenshotUpload')[0].files;
            fd.append('file', files[0]);
            fd.append('GUID', ScreenshotGUID);
            sendFileToServer(fd, url,continueId);
        }
